...
public class Arr2Ex006 {
	// 변수
	static String[][] users = new String[5][9];

	static String[] name = { "아이언맨", "헐크", "캡틴", "토르", "호크아이" };
	static int[] kor = { 100, 20, 90, 70, 35 };
	static int[] eng = { 100, 50, 95, 80, 100 };
	static int[] mat = { 100, 30, 90, 60, 100 };
	static int indexNum = 0;// 등수 처리를 위한 이중for문 안에서 쓸 변수
	...
	static void step1() {
		for (int i = 0; i < users.length; i++) {
			...
			users[i][4] = Integer.toString((kor[i] + eng[i] + mat[i]) / 3); // 평균 입력
			// 단순 입력 값들 처리 ENDb ====================================================
		}
	}
	...
	static void step2() {
		for (int i = 0; i < users.length; i++) {
			if (Integer.parseInt(users[i][4]) >= 70 && // 조건 평균 70 이상
	...
	static void step3() {
		for (int i = 0; i < users.length; i++) {
			if (Integer.parseInt(users[i][4]) >= 95) { // 조건 평균 95 이상
	...
	static void step4() {
		for (int i = 0; i < users.length; i++) {
			...
			for (int j = 0; j < Integer.parseInt(users[i][4]) / 10; j++) {
				users[i][8] += "#";
			} // 랭킹 처리 END ============================================================
...
//등수 처리#####################################################################
		...
		for (int i = 0; i < users.length; i++) {
			...
			for (int j = 0; j < users.length; j++) {
				if (Integer.parseInt(users[i][4]) <= Integer.parseInt(users[j][4])) {
